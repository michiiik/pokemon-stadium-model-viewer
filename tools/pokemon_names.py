"""Canonical English species names for the Stadium 1/2 model index range.

The model providers use the National Pokédex number as the stable identity
when the source record exposes one.  Stadium 1 consumes 1..151; Stadium 2
also contains the Johto species through 251 plus non-species records.
"""
...
def pokemon_name(model_id: object) -> str | None:
    """Return a canonical name for a 1-based National Pokédex number."""
    if isinstance(model_id, int) and 1 <= model_id <= len(POKEMON_NAMES):
        return POKEMON_NAMES[model_id - 1]
    return None
